...
if not f then   
if not redis:get(Server_Twix.."Token_DevTwix") then
...
redis:set(Server_Twix.."Token_DevTwix",token)
end 
else
...
------------------------------------------------------------------------------------------------------------
local DevTwix_Info_Sudo = io.open("Info_Sudo.lua", 'w')
DevTwix_Info_Sudo:write([[
do 
local Twix_INFO = {
Id_DevTwix = ]]..redis:get(Server_Twix.."Id_DevTwix")..[[,
UserName_Twix = "]]..redis:get(Server_Twix.."User_DevTwix1")..[[",
Token_Bot = "]]..redis:get(Server_Twix.."Token_DevTwix")..[["
}
return Twix_INFO
end

]])
DevTwix_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_Twix = io.open("Twix", 'w')
Run_File_Twix:write([[
#!/usr/bin/env bash
cd $HOME/Twix
token="]]..redis:get(Server_Twix.."Token_DevTwix")..[["
while(true) do
rm -fr ../.telegram-cli
...
os.execute('chmod +x Twix')
os.execute('chmod +x tk')
os.execute('./tk')
Status = true
else   
f:close()  
redis:del(Server_Twix.."Token_DevTwix");redis:del(Server_Twix.."Id_DevTwix");redis:del(Server_Twix.."User_DevTwix1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_Twix
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_DevTwix
Ids_Dev = {sudos.Id_DevTwix,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "تويكس"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_Twix(msg)  
local Dev_Twix = false  
...
function Dev_Twix_User(user)  
local Dev_Twix_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_Twix_User = true  
end  
end  
return Dev_Twix_User  
end 
function DeveloperBot(msg)  
local Status = redis:sismember(bot_id.."Developer:Bot", msg.sender_user_id_) 
if Status or Dev_Twix(msg) or Bot(msg) then  
return true  
else  
return false  
end  
...
if hash or Dev_Twix(msg) or DeveloperBot(msg) or Bot(msg) then  
return true 
else 
return false 
end 
...
if hash or Dev_Twix(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_Twix(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_Twix(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_Twix(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_Twix(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
return true 
else 
return false 
end 
...
local url , res = https.request('https://devglobla.cf/api/wawe.php/?id='..User..'') --- Developer :- @LGlobla 
data = JSON.decode(url)
if data.Ch_Member.ieeo3s  ~= true then
Var = false
else
Var = true
end
...
if tonumber(user_id) == tonumber(Id_Dev) then  
Status = true 
elseif Dev_Twix_User(user_id) then
Status = true  
...
if tonumber(user_id) == tonumber(Id_Dev) then  
Status = 'مطــور السـورس'
elseif Dev_Twix_User(user_id) == true then
...
------------------------------------------------------------------------------------------------------------
function keed(User_id,Chat_id)
https.request("https://api.telegram.org/bot"..token.."/keedMember?chat_id="..Chat_id.."&user_id="..User_id)
end
------------------------------------------------------------------------------------------------------------
...
function filter(msg, value)  
local names = redis:smembers(bot_id..'filters'..msg.chat_id_)  local var = false for i=1, #names do  if names[i] == value and not Vips(msg) then  var = true      end end return var  
end 
------------------------------------------------------------------------------------------------------------
function tdcli_update_callback_value(Data) 
url = 'https://raw.githubusercontent.com/TEAMTWIX/TWIX/master/Twix.lua'
file_path = 'Twix.lua'
local respbody = {} 
...
data.first_name_ = gmatch or 'Twix'
end
...
if Type == "keed" then
https.request("https://api.telegram.org/bot" .. token .. "/keedMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..msg.sender_user_id_.."") 
redis:sadd(bot_id.."Keed:User"..msg.chat_id_,msg.sender_user_id_) 
...
------------------------------------------------------------------------------------------------------------
function FilesTwix(msg)
File_Bot = dofile("Twix.lua")
if File_Bot.Twix and msg then
Text_File = File_Bot.Twix(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesTwixBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.TwixFile and msg then
Text_FileBot = Text_FileBot.TwixFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_Twix_File(msg,data)
if msg then
msg = data.message_
...
keed(msg.chat_id_,msg.sender_user_id_)
redis:sadd(bot_id.."Keed:User"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
...
sens = 400  
if redis:get(bot_id.."lock:Spam"..msg.chat_id_) == "del" and not Vips(msg) and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."lock:Spam"..msg.chat_id_) == "ked" and not Vips(msg) and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
keed(msg.chat_id_,msg.sender_user_id_)
redis:sadd(bot_id.."Keed:User"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."lock:Spam"..msg.chat_id_) == "kick" and not Vips(msg) and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."lock:Spam"..msg.chat_id_) == "ktm" and not Vips(msg) and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
redis:sadd(bot_id.."Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if tonumber(msg.sender_user_id_) == tonumber(1373615087) then
send(msg.chat_id_, msg.id_,'هلا حبيبي وتجراسي واوي .')
return false 
end
...
if Text_Filter then    
Send_Options(msg,msg.sender_user_id_,"","")  
Delete_Message(msg.chat_id_, {[0] = msg.id_})     
return false
end
end
...
--------------------------------------------------------
if text == ""..(redis:get(bot_id.."Status:Klam:engilsh"..msg.chat_id_) or "").."" and not redis:get(bot_id.."Status:Set:engilsh"..msg.chat_id_) then
if not redis:get(bot_id.."Status:Set:engilsh"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - انكلش")
redis:incrby(bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Status:Set:engilsh"..msg.chat_id_,true)
end 
-------------------------------------------------------------
if text == ""..(redis:get(bot_id.."Status:Klam:plus"..msg.chat_id_) or "").."" and not redis:get(bot_id.."Status:Set:plus"..msg.chat_id_) then
if not redis:get(bot_id.."Status:Set:plus"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - رياضيات")
redis:incrby(bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Status:Set:plus"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:DevTwix'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:DevTwix'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:DevTwix',text)
redis:del(bot_id..'GetTexting:DevTwix'..msg.chat_id_..':'..msg.sender_user_id_)
...
if Dev_Twix(msg) then
if text == 'نقل الاحصائيات' then
local Users = redis:smembers(bot_id.."Abs:Users")
local Groups = redis:smembers(bot_id.."Abs:Groups") 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'Twix:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:User"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'Twix:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'Twix:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'Twix:Mod:User'..Groups[i])
...
download_to_file('https://raw.githubusercontent.com/TEAMTWIX/TWIX/master/Twix.lua','Twix.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/TwixTeam/Files_Twix/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n⌔︙قائمه ملفات متجر سورس Twix\n⌔︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/TwixTeam/Files_Twix/master/Files_Twix/"..File_Name)
if Res == 200 then
...
if not Dev_Twix(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
...
redis:set(bot_id..'GetTexting:DevTwix'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:DevTwix')
...
if Dev_Twix_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..Twix..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
local Get_Json = '{"BOT_ID": '..bot_id..',"Groups":{'  
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_Twix(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
if text and text:match("^اضف مطور @(.*)$") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
if text and text:match("^حذف مطور @(.*)$") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
if text and text:match("^اضف مطور (%d+)$") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
if text and text:match("^حذف مطور (%d+)$") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
if text == 'جلب نسخه احتياطيه' and Dev_Twix(msg) or text == 'جلب نسخه الكروبات' and Dev_Twix(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd')  
local t = '{"BOT_ID": '..bot_id..',"Groups":{'  
for k,v in pairs(list) do   
NAME = 'Tshake Chat'
...
if text == ("مسح قائمه العام") and Dev_Twix(msg) or text == ("مسح المحظورين عام") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌔︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
elseif text == ("قائمه العام") and Dev_Twix(msg) or text == ("المحظورين عام") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
local List = redis:smembers(bot_id..'Keed:User'..msg.chat_id_)
for k,v in pairs(List) do   
...
redis:del(bot_id..'Keed:User'..msg.chat_id_, v)
end 
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
if Dev_Twix_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
redis:srem(bot_id.."Keed:User"..msg.chat_id_, result.sender_user_id_)
Send_Options(msg,result.sender_user_id_,"reply","⌔︙تم الغاء تقييده")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end
if text == ("تقيد") and msg.reply_to_message_id_ and Admin(msg) then
...
https.request("https://api.telegram.org/bot"..token.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..result.sender_user_id_)
redis:sadd(bot_id..'Keed:User'..msg.chat_id_, result.sender_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
Send_Options(msg,result.sender_user_id_,"reply","⌔︙تم تقيد العضو في المجموعة")
end,nil)
end
end
...
if text and text:match("^حظر عام @(.*)$") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
redis:sadd(bot_id..'Keed:User'..msg.chat_id_, result.id_)
...
https.request("https://api.telegram.org/bot"..token.."/keedMember?chat_id="..msg.chat_id_.."&user_id="..result.id_..'&until_date='..tonumber(msg.date_+Time))
end
end
end
...
if (result.id_) then
https.request("https://api.telegram.org/bot" .. token .. "/keedMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. result.id_ .. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
if Dev_Twix_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_Twix(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @ieeo3s')   
return false
end
...
https.request("https://api.telegram.org/bot" .. token .. "/keedMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..text:match("^تقيد (%d+)$"))
...
https.request("https://api.telegram.org/bot"..token.."/keedMember?chat_id="..msg.chat_id_.."&user_id="..result.sender_user_id_..'&until_date='..tonumber(msg.date_+Time))
end
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
...
https.request("https://api.telegram.org/bot" .. token .. "/keedMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..text:match("^الغاء تقيد (%d+)$").. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
...
redis:set(bot_id.."Status:Lock:tagservrbot"..msg.chat_id_,true)   
list ={"Status:lock:Far","Status:Lock:Bot:kick","Status:Lock:User:Name","Status:Lock:hashtak","Status:Lock:Cmd","Status:Lock:Link","Status:Lock:forward","Status:Lock:Keyboard","Status:Lock:geam","Status:Lock:Photo","Status:Lock:Animation","Status:Lock:Video","Status:Lock:Audio","Status:Lock:vico","Status:Lock:Sticker","Status:Lock:Document","Status:Lock:Unsupported","Status:Lock:Markdaun","Status:Lock:Contact","lock:Spam"}
for i,lock in pairs(list) do;redis:set(bot_id..lock..msg.chat_id_,"del");end
redis:set(bot_id.."Status:lockpin"..msg.chat_id_, true) 
redis:sadd(bot_id.."Status:Lock:pin",msg.chat_id_) 
tdcli_function ({ ID = "GetChannelFull",  channel_id_ = msg.chat_id_:gsub("-100","") }, function(arg,data)  redis:set(bot_id.."Get:Id:Msg:Pin"..msg.chat_id_,data.pinned_message_id_)  end,nil)
...
elseif text == "تفعيل المغادره" and Dev_Twix(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_Twix(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_Twix(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_Twix(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_Twix(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_Twix(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_Twix(msg) then
redis:set(bot_id..'GetTexting:DevTwix'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_Twix(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
if text and text:match('^منع (.*)') and Admin(msg) then 
local text = text:match('^منع (.*)') 
if redis:sismember(bot_id..'filters'..msg.chat_id_,text) then
send(msg.chat_id_, msg.id_,'*⌔︙ الكلمه » ❪ '..text..' ❫*\n*⌔︙* بالتأكيد تم منع هاذه الكلمه ') 
else
redis:sadd(bot_id..'filters'..msg.chat_id_,text) 
send(msg.chat_id_, msg.id_,'*⌔︙ الكلمه » ❪ '..text..' ❫*\n*⌔︙* تم منع ارسال هاذه الكلمه ') 
end
end 
if text and text:match('^الغاء المنع (.*)')  and Admin(msg) then  
local text = text:match('^الغاء المنع (.*)') 
if not redis:sismember(bot_id..'filters'..msg.chat_id_,text) then
send(msg.chat_id_, msg.id_,'*⌔︙ الكلمه » ❪ '..text..' ❫*\n*⌔︙* بالتأكيد تم الغاء منع الكلمه') 
else
redis:srem(bot_id..'filters'..msg.chat_id_,text) 
send(msg.chat_id_, msg.id_,'*⌔︙ الكلمه » ❪ '..text..' ❫*\n*⚜¦* تم الغاء منع هاذه الكلمه') 
end
elseif text and text:match('^الغاء منع (.*)') and Admin(msg) then  
local text = text:match('^الغاء منع (.*)')
if not redis:sismember(bot_id..'filters'..msg.chat_id_,text) then
send(msg.chat_id_, msg.id_, '*⌔︙ الكلمه » ❪ '..text..' ❫*\n*⌔︙* بالتأكيد تم الغاء منع الكلمه') 
else
redis:srem(bot_id..'filters'..msg.chat_id_,text) 
...
if text ==('مسح قائمه المنع') and Admin(msg) then redis:del(bot_id..'filters'..msg.chat_id_) 
send(msg.chat_id_, msg.id_,'⌔︙ تم مسح قائمه المنع') end 
if text == ('قائمه المنع') and Admin(msg) then  
local list = redis:smembers(bot_id..'filters'..msg.chat_id_) 
local t = '⌔︙ قائمه منع الكلمات  \n--------------------\n' for k,v in pairs(list) do 
t = t..'*'..k.."» { * `"..v.."` *} *\n"  
end 
if #list == 0 then t = '⌔︙ قائمه المنع فارغه' end 
send(msg.chat_id_, msg.id_,t) 
end 
...
elseif text == ("مسح ردود المطور") and Dev_Twix(msg) then 
...
elseif text == ("ردود المطور") and Dev_Twix(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_Twix(msg) then 
...
elseif text == "حذف رد للكل" and Dev_Twix(msg) then 
...
if Dev_Twix_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
...
elseif text == "تغير اسم البوت" and Dev_Twix(msg) or text == "تغيير اسم البوت" and Dev_Twix(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_Twix(msg) then 
...
lock_spam = "← {✔️}"
elseif redis:get(bot_id.."Keed:User"..msg.chat_id_) == "ked" then 
lock_spam = "← { بالتقيد }"    
...
if text == ("رفع اثول") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⌔︙ تم رفع العضو اثول في المجموعة 😭😹💞")  
end, nil)
elseif text == ("تنزيل اثول") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم تنزيل العضو اثول في المجموعة 😭😹💞")  
end, nil)
elseif text == ("رفع نوتيلا") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⌔︙تم رفع العضو نوتلا في المجموعة 😻💞")  
end, nil)
elseif text == ("رفع تويكس") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⌔︙تم رفع العضو تويكس في المجموعة 😻💞")  
end, nil)
elseif text == ("رفع شوكلاته") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⌔︙تم رفع العضو شوكلاته في المجموعة 😻🍫💞")  
end, nil)
elseif text == ("رفع ورده") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⌔︙تم رفع العضو ورده في المجموعة منورنا ياورده😭😹💞")  
end, nil)
elseif text == ("رفع جلب") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⌔︙تم رفع العضو جلب في المجموعة 😭😹💞")  
end, nil)
elseif text == ("تنزيل جلب") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم تنزيل العضو جلب في المجموعة 😭😹💞")  
end, nil)
elseif text == ("رفع مطي") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم رفع العضو مطي في المجموعة 😭😹💞")  
end, nil)
elseif text == ("تنزيل مطي") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم تنزيل العضو مطي في المجموعة 😭😹💞")  
end, nil)
elseif text == ("رفع صخل") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم رفع العضو صخل في المجموعة 😭😹💞")  
end, nil)
elseif text == ("تنزيل صخل") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم تنزيل العضو صخل في المجموعة 😭😹💞")  
end, nil)
elseif text == ("رفع زاحف") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم رفع العضو زاحف في المجموعة 😭😹💞")  
end, nil)
elseif text == ("تنزيل زاحف") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم تنزيل العضو زاحف في المجموعة 😭😹💞")  
end, nil)
elseif text == ("رفع بكلبي") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم رفع العضو بكلبي في المجموعة 😭😹💞")  
end, nil)
elseif text == ("تنزيل بكلبي") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم تنزيل العضو بكلبي في المجموعة 😭😹💞")  
end, nil)
elseif text == ("رفع الرفع") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم رفع العضو الرافع في المجموعة 😭😹💞")  
end, nil)
elseif text == ("تنزيل الرفع") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم تنزيل العضو الرافع في المجموعة 😭😹💞")  
end, nil)
elseif text == ("رفع تاج") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم رفع العضو تاج في المجموعة 😭😹💞")  
end, nil)
elseif text == ("تنزيل تاج") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⋄︙تم تنزيل العضو تاج في المجموعة 😭😹💞")  
end, nil)
elseif text == ("رفع نبي") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
send(msg.chat_id_, msg.id_,"⌔︙رفعتلكم نبي جديد استقبلو 😹😭💞")  
end, nil)
elseif text == ("تنزيل نبي") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, function(extra, result, success)
...
local Texting = {"حلغوم والله ،🥺💘","لزكت بيها دغيرها عاد ،😒😕","صورتك مامرتاحلها ،🙄😶", "ملاك وناسيك بكروبنه ،🤤💞"}
...
elseif text == "انكليزي" or text == "انكلش" then
if redis:get(bot_id.."Status:Lock:Game:Group"..msg.chat_id_) then
redis:del(bot_id.."Status:Set:engilsh"..msg.chat_id_)
engilsh = {'معلومات','قنوات','مجموعات','كتاب','تفاحه','صداقه','نقود','اعلم','ذئب','تمساح','ذكي','شاطئ','غبي',};
name = engilsh[math.random(#engilsh)]
redis:set(bot_id.."Status:Klam:engilsh"..msg.chat_id_,name)
...
send(msg.chat_id_, msg.id_,"⌔︙ما معنى كلمة ↓\n {"..name.."}")
return false
end
elseif text == "الرياضيات" or text == "رياضيات" then
if redis:get(bot_id.."Status:Lock:Game:Group"..msg.chat_id_) then
redis:del(bot_id.."Status:Set:plus"..msg.chat_id_)
plus = {'9','46','2','9','5','4','25','10','17','15','39','5','16',};
name = plus[math.random(#plus)]
redis:set(bot_id.."Status:Klam:plus"..msg.chat_id_,name)
...
send(msg.chat_id_, msg.id_,"⌔︙اكمل المعادله التاليه ↓\n {"..name.."}")
return false
end
...
𓂅 .Twix TEAM 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
𓂅 . [Source Channel](t.me/ieeo3s)

𓂅 . [Source Info ](t.me/TKSLX)          
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
𓂅 . [TWS Twix](t.me/PPPPPE)     
]]) 
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_Twix(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_Twix(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
redis:srem(bot_id.."Removal:User:Group"..msg.chat_id_,result.id_)
redis:srem(bot_id.."Silence:User:Group"..msg.chat_id_,result.id_)
redis:srem(bot_id.."Keed:User"..msg.chat_id_,result.id_)
...
if redis:sismember(bot_id.."Keed:User"..msg.chat_id_,result.id_) then
Zain = "مقيد"
else
Zain = "غير مقيد"
end
...
if redis:sismember(bot_id.."Keed:User"..msg.chat_id_,result.sender_user_id_) then
Zain = "مقيد"
else
Zain = "غير مقيد"
end
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_Twix(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevTwix = redis:get(bot_id..'Texting:DevTwix')
if TextingDevTwix then 
send(msg.chat_id_, msg.id_,TextingDevTwix)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_Twix(msg) then
redis:del(bot_id..'Texting:DevTwix')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_Twix(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_Twix_File(msg,data)
FilesTwixBot(msg,data)
...
------------------------------------------------------------------------
if text then   
    if filter(msg,text) then    
        Delete_Message(msg.chat_id_,{[0] = msg.id_})        
    return false end   
    end  
    
    local txxt = ta.content_.text_
    if txxt and not Vips(ta) then  
    if filter(ta,txxt) then    
        Delete_Message(data.chat_id_,{[0] = data.message_id_}) 
    end end
